...
if [ -d ${BASE_DIR}/workbase/overlaydir_strip ]; then
	if [ -d ${NEW_TARGET_SYSDIR}/scripts/os_first_run/ ]; then
		...
		do
			...
			if [ -f ${BASE_DIR}/workbase/overlaydir/${OVERLAY_DIR}.released ]; then
				continue;
			fi
	...
	if [ -d ${NEW_TARGET_SYSDIR}/scripts/os_start_run/ ]; then
		for dir in $(find ${NEW_TARGET_SYSDIR}/scripts/os_start_run/ -name "*.run" | sed "s@${NEW_TARGET_SYSDIR}/scripts/os_start_run/@@g" | awk -F'.' '{ print $1 }' | sort | uniq)
		do
			if [ ! -f ${BASE_DIR}/env/${dir}/overlay.set ]; then
				echo "${dir} 组没有定义overlay目录，无法安装其中软件包相关的首次启动执行脚本。"
				exit 1
	...
	if [ -d ${NEW_TARGET_SYSDIR}/scripts/update/os_first_run/ ]; then
		for dir in $(find ${NEW_TARGET_SYSDIR}/scripts/update/os_first_run/ -name "*.run" | sed "s@${NEW_TARGET_SYSDIR}/scripts/update/os_first_run/@@g" | awk -F'.' '{ print $1 }' | sort | uniq)
		do
			if [ ! -f ${BASE_DIR}/env/${dir}/overlay.set ]; then
				echo "${dir} 组没有定义overlay目录，无法安装其中软件包相关的首次启动执行脚本。"
				exit 1
			...
			if [ ! -d ${BASE_DIR}/workbase/overlaydir_strip/${OVERLAY_DIR}.update/etc/first-run.d/ ]; then
				mkdir -pv ${BASE_DIR}/workbase/overlaydir_strip/${OVERLAY_DIR}.update/etc/first-run.d/
			fi
			for run_file in $(ls ${NEW_TARGET_SYSDIR}/scripts/update/os_first_run/${dir}.*.run | sed "s@${NEW_TARGET_SYSDIR}/scripts/update/os_first_run/${dir}\.@@g" | awk -F'.' '{ print $1 }')
			do
				echo -n "正在安装${dir}.${run_file}.run文件到 ${OVERLAY_DIR}.update/etc/first-run.d/51-$(date +%Y%m%d%H%M%S)-${dir}_${run_file}.sh ..."
				cat ${NEW_TARGET_SYSDIR}/scripts/update/os_first_run/${dir}.${run_file}.run > ${BASE_DIR}/workbase/overlaydir_strip/${OVERLAY_DIR}.update/etc/first-run.d/51-${run_file}_${dir}_$(date +%Y%m%d%H%M%S).sh
				echo "完成！"
			done
		done
	fi
	...
	if [ -d ${NEW_TARGET_SYSDIR}/scripts/update/os_start_run/ ]; then
		for dir in $(find ${NEW_TARGET_SYSDIR}/scripts/update/os_start_run/ -name "*.run" | sed "s@${NEW_TARGET_SYSDIR}/scripts/update/os_start_run/@@g" | awk -F'.' '{ print $1 }' | sort | uniq)
		do
			if [ ! -f ${BASE_DIR}/env/${dir}/overlay.set ]; then
				echo "${dir} 组没有定义overlay目录，无法安装其中软件包相关的首次启动执行脚本。"
				exit 1
			...
			if [ ! -d ${BASE_DIR}/workbase/overlaydir_strip/${OVERLAY_DIR}.update/etc/start-run.d/ ]; then
				mkdir -pv ${BASE_DIR}/workbase/overlaydir_strip/${OVERLAY_DIR}.update/etc/start-run.d/
			fi
			for run_file in $(ls ${NEW_TARGET_SYSDIR}/scripts/update/os_start_run/${dir}.*.run | sed "s@${NEW_TARGET_SYSDIR}/scripts/update/os_start_run/${dir}\.@@g" | awk -F'.' '{ print $1 }')
			do
				echo -n "正在安装${dir}.${run_file}.run文件到 ${OVERLAY_DIR}.update/etc/start-run.d/51-$(date +%Y%m%d%H%M%S)-${dir}_${run_file}.sh ..."
				cat ${NEW_TARGET_SYSDIR}/scripts/update/os_start_run/${dir}.${run_file}.run > ${BASE_DIR}/workbase/overlaydir_strip/${OVERLAY_DIR}.update/etc/start-run.d/51-${run_file}_${dir}_$(date +%Y%m%d%H%M%S).sh
				echo "完成！"
			done
		done
	fi
