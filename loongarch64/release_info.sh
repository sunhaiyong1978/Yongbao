...
touch ${BASE_DIR}/workbase/logs/release_show.txt
if [ -f ${BASE_DIR}/workbase/logs/update_release_show.txt ]; then
	mv ${BASE_DIR}/workbase/logs/update_release_show.txt{,.$(date +%Y%m%d%H%M%S)}
fi
touch ${BASE_DIR}/workbase/logs/update_release_show.txt
...
if [ -f ${BASE_DIR}/workbase/logs/update_release_info.txt ]; then
	mv ${BASE_DIR}/workbase/logs/update_release_info.txt{,.$(date +%Y%m%d%H%M%S)}
	if [ -f ${BASE_DIR}/workbase/logs/update_release_info.temp ]; then
		rm ${BASE_DIR}/workbase/logs/update_release_info.temp
		touch ${BASE_DIR}/workbase/logs/update_release_info.temp
	fi
fi
...
if [ -f ${BASE_DIR}/workbase/logs/update_release_summary.txt ]; then
	mv ${BASE_DIR}/workbase/logs/update_release_summary.txt{,.$(date +%Y%m%d%H%M%S)}
fi
...
do
	...
	if [ -d ${BASE_DIR}/workbase/overlaydir_strip/${i}.update ]; then
		echo "统计 ${i}.update 目录内的软件版本信息..."
		echo "${i} 组中更新或增加了以下软件包：" >> ${BASE_DIR}/workbase/logs/update_release_info.txt
		for pkg_info in $(ls ${BASE_DIR}/workbase/overlaydir_strip/${i}.update/var/Yongbao/status/* )
		do
			if [ -f ${pkg_info} ]; then
				if [ "x$(grep "^${i}/${pkg_info##*/}" ${BASE_DIR}/info_set/release_hide)" == "x" ] && [ "x$(grep "^${pkg_info##*/}$" ${BASE_DIR}/info_set/release_hide)" == "x" ]; then
					if [ "x$(cat ${BASE_DIR}/info_set/release_summary | grep "^${pkg_info##*/}=")" != "x" ] && [ "x$(cat ${BASE_DIR}/workbase/logs/update_release_info.temp | grep "^${pkg_info##*/}-$(cat ${pkg_info})$")" == "x" ]; then
						echo "找到一个提要${pkg_info##*/}。"
						echo "${pkg_info##*/}-$(cat ${pkg_info})" >> ${BASE_DIR}/workbase/logs/update_release_info.temp
						echo "${pkg_info##*/}=$(cat ${BASE_DIR}/info_set/release_summary | grep "^${pkg_info##*/}=" | awk -F'=' '{ print $2 }' | sed "s@<<<VERSION>>>@$(cat ${pkg_info})@g")" >> ${BASE_DIR}/workbase/logs/update_release_show.txt
					fi
					echo "	${pkg_info##*/} $(cat ${pkg_info})" >> ${BASE_DIR}/workbase/logs/update_release_info.txt
				fi
			fi
		done
		echo "" >> ${BASE_DIR}/workbase/logs/update_release_info.txt
	fi
...
do
	...
	if [ "x$(cat ${BASE_DIR}/workbase/logs/update_release_show.txt | grep "^${i##*/}=")" != "x" ]; then
		echo "* $(cat ${BASE_DIR}/workbase/logs/update_release_show.txt | grep "^${i##*/}=" | awk -F'=' '{ print $2 }')" >> ${BASE_DIR}/workbase/logs/update_release_summary.txt
	fi
done
